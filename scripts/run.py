...
def _game_args(extra_args: list[str] | None) -> list[str]:
...
def run(project_dir: str, mode: str, platform: str, extra_args: list[str] | None = None) -> int:
    ...
    if mode == "debug":
        ...
        cmd = [_resolve_godot_bin(), "--path", str(project_path)] + game_args
    ...
    if mode == "release":
        ...
        subprocess.run([str(exe_path)] + game_args, cwd=str(build_dir), check=True)
        return 0
